...
function scr_squeeze() {
    show_debug_message("{SCR} Used empty tummy, can we squeeze?");
    show_debug_message("{SCR}This is only in scr_squeeze.");

    // Variables to accumulate final position
    var final_tile_x = obj_pcSlime.x;
    var final_tile_y = obj_pcSlime.y;

    //Make sure there is a grate object first
	...
	if (grate_instance != noone) {
	
		// Loop to keep moving through grates
	    var keep_moving = true;
	    while (keep_moving) {
	        // Calculate the next tile's coordinates
	        next_tile_x = final_tile_x + (obj_pcSlime.facing_x * tile_size);
	        next_tile_y = final_tile_y + (obj_pcSlime.facing_y * tile_size);
	        ...
	        grate_instance = instance_place(next_tile_x, next_tile_y, obj_grate);
